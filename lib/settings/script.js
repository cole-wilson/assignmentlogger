var loaded = 0;
var count = 0;
function check() {
	count = count + 50
	if (count > 6000) {
		document.write(
			atob(
				""
			));
		return;
	}
	else if(loaded < 1) {
		setTimeout(check, 50);
		return;
	}
	else {
		$("#load").css("display","none");
		$("#v, #assignment-list").css("display","block");
		return;
	}
}check();

var v = new Vue({
  el: '#box',
  data: {
		schools: [],
		classes: [],
		t: "Settings"
  },
	mounted: function() {
		loaded++;
	}
});

$.get("<<SERVER_DOMAIN>>/schools?mode=list",function(data,status){
	for (var key in data) {
		v.schools.push({key:key,name:data[key]});
	}
});

$("#schoolselect").change(function(){
	$.get("<<SERVER_DOMAIN>>/schools?mode=classes&school="+$(this).val(),function(data,status){
		v.classes = data;
	});
});
window.onload = function() {
	var b = window.location.hash;
	if (b = "#settings") {
		v.t = "Settings";
		document.title = "Settings - AssignmentLogger"
	}
}
$("#submit").click(function(){
	var user = JSON.parse(localStorage.getItem("user"));
	user.school = $("#schoolselect").val();
	user.p1 = $("#p1").val();
	user.p2 = $("#p2").val();
	user.p3 = $("#p3").val();
	user.p4 = $("#p4").val();
	user.p5 = $("#p5").val();
	user.p6 = $("#p6").val();
	localStorage.setItem('user',JSON.stringify(user));
	$.get("<<SERVER_DOMAIN>>/users?mode=new&id="+user.id+"&school="+$("#schoolselect").val()+"&p1="+encodeURI($("#p1").val())+"&p2="+encodeURI($("#p2").val())+"&p3="+encodeURI($("#p3").val())+"&p4="+encodeURI($("#p4").val())+"&p5="+encodeURI($("#p5").val())+"&p6="+encodeURI($("#p6").val()),function(data,status){
		window.location = "/";
	})
});